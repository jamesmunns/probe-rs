...
impl ProbeFactory for CmsisDapFactory {
    async fn open(
        ...
        Ok(Box::new(
            CmsisDap::new_from_device(tools::open_device_from_selector(&selector).await?).await?,
...
impl CmsisDap {
    ...
    async fn jtag_reset_scan(&mut self) -> Result<(BitVec<u8>, BitVec<u8>), CmsisDapError> {
    ...
    async fn jtag_scan_ir(&mut self) -> Result<BitVec<u8>, CmsisDapError> {
    ...
    /// the Run-Test/Idle state.
    async fn jtag_scan_dr(&mut self) -> Result<BitVec<u8>, CmsisDapError> {
    ...
    async fn jtag_scan_inner(&mut self, name: &'static str) -> Result<BitVec<u8>, CmsisDapError> {
    ...
    async fn batch_add(&mut self, command: BatchCommand) -> Result<Option<u32>, ArmError> {
        ...
        match command {
            BatchCommand::Read(_, _) => self.process_batch().await,
            _ if self.batch.len() == max_writes => self.process_batch().await,
            _ => Ok(None),
        }
    }
...
impl RawDapAccess for CmsisDap {
    ...
    async fn raw_read_register(&mut self, port: PortType, addr: u8) -> Result<u32, ArmError> {
        let res = self
            .batch_add(BatchCommand::Read(port, addr as u16))
            .await?;
