...
impl CoreDump {
    ...
    pub async fn dump_core(core: &mut Core<'_>, ranges: Vec<Range<u64>>) -> Result<Self, Error> {
        let instruction_set = core.instruction_set().await?;
        let core_type = core.core_type();
        let supports_native_64bit_access = core.supports_native_64bit_access().await;
        let fpu_support = core.fpu_support().await?;
