...
impl<'probe> Core<'probe> {
    ...
    async fn find_free_breakpoint_comparator_index(&mut self) -> Result<usize, Error> {
        let mut next_available_hw_breakpoint = 0;
        for breakpoint in self.inner.hw_breakpoints().await? {
    ...
    pub async fn set_hw_breakpoint(&mut self, address: u64) -> Result<(), Error> {
            ...
            .position(|&bp| bp == Some(address))
        {
            ...
            None => self.find_free_breakpoint_comparator_index().await?,
        };
...
impl<'probe> CoreInterface for Core<'probe> {
    ...
    async fn set_hw_breakpoint(&mut self, _unit_index: usize, addr: u64) -> Result<(), Error> {
        self.set_hw_breakpoint(addr).await
    }

    async fn clear_hw_breakpoint(&mut self, _unit_index: usize) -> Result<(), Error> {
        self.clear_all_hw_breakpoints().await
    }
