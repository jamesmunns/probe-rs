...
pub trait ArmDebugSequence: Send + Sync + Debug {
    ...
    async fn reset_hardware_deassert(
        ...
        let can_read_pins = memory.swj_pins(n_reset, n_reset, 0).await? != 0xffff_ffff;

        if can_read_pins {
            let start = Instant::now();

            loop {
                if Pins(memory.swj_pins(n_reset, n_reset, 0).await? as u8).nreset() {
                    return Ok(());
                }
    ...
    async fn debug_port_start(
        ...
        interface.write_dp_register(dp, Select(0)).await?;
    ...
    async fn debug_port_stop(
        ...
        // Select Bank 0
        interface
            .raw_write_register(PortType::DebugPort, Select::ADDRESS, 0)
            .await?;

        // De-assert debug power request
        interface
            .raw_write_register(PortType::DebugPort, Ctrl::ADDRESS, 0)
            .await?;
    ...
    async fn debug_port_connect(
        ...
        interface
            .raw_write_register(PortType::DebugPort, Abort::ADDRESS, abort.0)
            .await?;
        ...
        if let DpAddress::Multidrop(targetsel) = dp {
            ...
            // Select DP Bank 2
            interface
                .raw_write_register(PortType::DebugPort, Select::ADDRESS, 2)
                .await?;

            let target_id = interface
                .raw_read_register(PortType::DebugPort, TARGETID::ADDRESS & 0xf)
                .await?;

            // Select DP Bank 3
            interface
                .raw_write_register(PortType::DebugPort, Select::ADDRESS, 3)
                .await?;
            let dlpidr = interface
                .raw_read_register(PortType::DebugPort, DLPIDR::ADDRESS & 0xf)
                .await?;
