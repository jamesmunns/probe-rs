...
where
    DP: DapAccess,
{
    let mut fqaa = Vec::new();
    for ap in allowed_aps.into_iter() {
        let ap = FullyQualifiedApAddress::v1_with_dp(dp, ap);
        if let Some(ap) = access_port_is_valid(debug_port, &ap).await.map(|_| ap) {
            fqaa.push(ap);
        } else {
            break;
        }
    ...
    P: Fn(IDR) -> bool,
{
    for ap in (0..=255).map(|ap| GenericAp::new(FullyQualifiedApAddress::v1_with_dp(dp, ap))) {
        if let Ok(true) = debug_port.read_ap_register(&ap).await.map(&f) {
            return Some(ap);
        }
    }
    None
